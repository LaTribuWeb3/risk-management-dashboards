...
selectedPoolData(tab) {
  ...
  const tokenData = Object.assign([], this['data/tokens?fakeMainnet=0_data'] || []);
  const creditAccountData = Object.assign([], this['data/creditAccounts?fakeMainnet=0_data'] || []);
  alertStore.valueAtRisk = tab;

  const selectedPool = poolsData.find(p => p.address === tab);
  const creditAccountsForPool = creditAccountData.filter(ca => ca.poolAddress === tab);
